# 🐢 T-Guides: Гайды и тактики для сервера Turtle WoW

[![Last commit](https://img.shields.io/github/last-commit/whtmst/T-Guides?style=flat-square&label=Last%20updated&labelColor=212830&color=80c101)](https://github.com/whtmst/T-Guides/commits/main)
[![License](https://img.shields.io/github/license/whtmst/T-Guides?style=flat-square&label=License&labelColor=212830&color=f5c62c)](https://github.com/whtmst/T-Guides/blob/main/LICENSE)

> 💡 **О проекте:** Этот репозиторий содержит полезные ссылки, подробные тактики и гайды для рейдов и подземелий сервера **[Turtle WoW](https://turtle-wow.org/)**.

---

## 🧭 Содержание

### 🧭 Подземелья (DUNGEONS)

| Название | Уровень | Статус | Ссылка |
| :--- | :--- | :--- | :--- |
| **Lower Blackrock Spire (LBRS)** | 55-60 | 🛠️ В разработке | |
| **Upper Blackrock Spire (UBRS)** | 55-60 | 🛠️ В разработке | |
| **Karazhan Crypt (Склепы Каражана)** | 58-60 | 🛠️ В разработке | |
| **Stormwind Vault (Хранилище Штормграда)** | 60 | 🛠️ В разработке | |
| **Caverns of Time: Black Morass (Пещеры времени: Черные топи)** | 60 | ✅ Готов | [DUNGEONS/Black Morass.md](DUNGEONS/Black%20Morass.md) |

---

### 💀 Рейды (RAIDS)

| Название | Уровень | Статус | Ссылка |
| :--- | :--- | :--- | :--- |
| **Onyxia's Lair (Ony)** | 40-Man | 🛠️ В разработке | |
| **Molten Core (MC)** | 40-Man | ✅ Готов | [RAIDS/Molten Core.md](RAIDS/Molten%20Core.md) |
| **Lower Karazhan Halls (LKH)** | 10-Man | ✅ Готов | [RAIDS/Lower Karazhan Halls.md](RAIDS/Lower%20Karazhan%20Halls.md) |
| **Blackwing Lair (BWL)** | 40-Man | 🛠️ В разработке | |
| **Emerald Sanctum (ES)** | 40-Man | 🛠️ В разработке | |
| **Zul'Gurub (ZG)** | 20-Man | 🛠️ В разработке | |
| **Ruins of Ahn'Qiraj (AQ20)** | 20-Man | 🛠️ В разработке | |
| **Temple of Ahn'Qiraj (AQ40)** | 40-Man | 🛠️ В разработке | |
| **Naxxramas (Naxx)** | 40-Man | 🛠️ В разработке | |
| **Tower of Karazhan (TK)** | 40-Man | 🛠️ В разработке | |
| **Timbermaw Hold (TH)** | 20-Man | 🛠️ В разработке | |

---

## 🔗 🛠️ Инструментарий рейдера

Подборка инструментов, аддонов и сайтов для эффективного рейдинга и игры на сервере.

### 📊 Логи и аналитика
* **[Turtlogs](https://turtlogs.com/)** - сайт для загрузки и анализа логов.
* **[SuperWoW Combat Logger](https://github.com/pepopo978/SuperWowCombatLogger)** - инструмент для записи логов.
    * *Инструкция:* Требуется Python и SuperWoW. [Гайд по установке SuperWoW (RU)](https://github.com/whtmst/SuperwowInstallationRuGuide) | [Подготовка логов к выгрузке](https://github.com/pepopo978/SuperWowCombatLogger?tab=readme-ov-file#preparing-for-upload)
* **[Raid Charts Generator](https://whtmst.github.io/twow-raid-charts/)** - генератор графиков на основе логов.
    * *Инструкция:* [Как парсить данные для графиков](https://github.com/whtmst/twow-raid-charts)

### 📅 Планирование и теория
* **[Turtle WoW Talent Calculator](https://talent-builder.haaxor1689.dev/c/1.18.0)** - калькулятор талантов под патч 1.18.
* **[Turtle WoW Gear Planner](https://pbrigade.gitlab.io/gear-planner/)** - планировщик экипировки. Присутствуют не все классы.
* **[Threat Reference Table](https://www.warcrafttavern.com/wow-classic/guides/threat-guide-reference-table/)** - таблица механик угрозы (Threat) в классике.
* **[Turtle WoW Attunement Guide](https://forum.turtle-wow.org/viewtopic.php?t=17691)** - гайд по всем аттюнам (доступам) классики и черепахи.
* **[Raid Plan (Classic)](https://raidplan.io/classic)** - позиционирование и планирование босс-файтов.
* **[Raid Composition](https://www.wowhead.com/classic/raid-composition)** - состав рейда (Wowhead).
* **[Turtle WoW RaidRes](https://raidres.top/)** - инструмент для создания HR/SR списков.

### 🛠️ Аддоны и модификации
* **[BigWigs (Turtle WoW Edition)](https://github.com/pepopo978/BigWigs)** - боссмод, адаптированный под Turtle WoW.
* **[TWoW Raid Visuals](https://github.com/MarcelineVQ/twow-raid-visuals)** - визуальное отображение AOE-зон боссов.
    * *Важно:* Использовать без HD-модов. Может конфликтовать со старыми HD-модами. Если любите HD-моды, то рекомендую использовать новые [модульные HD-моды](https://forum.turtle-wow.org/viewtopic.php?t=21355).
* **[Модульные HD-моды](https://forum.turtle-wow.org/viewtopic.php?t=21355)** - полностью рабочие HD-моды (включают в себя адаптированный и рабочий мод TWoW Raid Visuals).
    * *Важно:* Читайте инструкцию внимательно, вам **ОБЯЗАТЕЛЬНО** нужно установить **[VanillaHelpers.dll](https://github.com/isfir/VanillaHelpers/releases)** инъекцию для того чтобы обеспечить нужное кол-во памяти для модов.
* **[Expressway Font](https://github.com/whtmst/T-Fonts)** - замена шрифтов на Expressway с полной поддержкой кириллицы.

### 🌍 Серверные утилиты
* **[Ambershire Roadmap](https://ambershire.pages.dev/)** - дорожная карта фаз сервера Ambershire.
    * Если недоступен сайт выше, то вот [зеркало](https://whtmst.github.io/ambershire-phase-roadmap/).
* **[Ambershire Timer](https://telabim-timer.deno.dev/#ambershire)** - таймеры КД мировых событий и боссов.
* **[Turtle Timer](https://turtle-timer.vercel.app/)** - альтернативный таймер (также есть [аддон для игры](https://github.com/sica42/TurtleCalendar)).
* **[Server Population](https://twow-pop.click/?realm=Ambershire)** - статистика онлайна (выбран Ambershire).

---

## 📦 Авторские аддоны (by Wht Mst)

Мои разработки для улучшения QoL в рейдах и не только:

1. **[T-Bidder (RU)](https://github.com/whtmst/T-Bidder)** - аддон для удобного участия в DKP-аукционах.
    * *Требование:* Необходима установка [этих шрифтов](https://github.com/whtmst/T-Fonts).
    * *Альтернатива:* [Версия с латиницей](https://github.com/whtmst/T-Bidder/tree/english) не требующая установки шрифтов.
2. **[T-RollTracker](https://github.com/whtmst/T-RollTracker)** - продвинутый трекер роллов в чате с объявлением победителя.
3. **[T-CombatLoggingReminder](https://github.com/whtmst/T-CombatLoggingReminder)** - авто-напоминание включить запись логов при входе в рейд.
4. **[T-OoM](https://github.com/whtmst/T-OoM)** - анонс окончания маны в чат.
5. **[T-Vamp](https://github.com/whtmst/T-Vamp)** - трекер эффективности статов вампиризма и отхила.
6. **[T-RestedXP](https://github.com/whtmst/T-RestedXP)** - отслеживание уровней Rested XP (0% и 100%).

---

## 📄 Лицензия

Лицензия [GPL-3.0 license](LICENSE) - не стесняйтесь использовать эти данные в своих проектах!

---

## 👥 Автор

**Создано и/или поддерживается пользователем**: [Wht Mst](https://github.com/whtmst)

*Сделано с ❤️ для сообщества Turtle WoW*

*Этот проект не связан (или не аффилирован) с Turtle WoW или Blizzard Entertainment.*


